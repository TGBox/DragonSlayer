package depr.old.text.newTextBasedVersion.meta;

import depr.old.text.newTextBasedVersion.classes.Character;
import depr.old.text.newTextBasedVersion.classes.Field;
import depr.old.text.newTextBasedVersion.classes.Item;
import depr.old.text.newTextBasedVersion.classes.Map;
...
public class TextGameHandler {
  ...
  private static final String[] GENDERS = new String[]{"Male", "Female", "Transgender",
      "Apache Helicopter"};
  ...
  public static void main(String[] args) {
    ...
    currentMapID++;
    player = new Character(name, gender, difficulty, map.getStartingPointX(),
        map.getStartingPointY());
    initialGameStart();
  }
  ...
  private static void initialGameStart() {
    System.out.println(
        "You wake up, lying on a " + currentField().getName() + ".\nYour only memories are, " +
  ...
  private static void additionalGameStart() {
    ...
    map = new Map(randomIntInRange(MIN_WIDTH_MAP, MAX_WIDTH_MAP),
        randomIntInRange(MIN_HEIGHT_MAP, MAX_HEIGHT_MAP), difficulty, currentMapID,
        player.getLevel());
    //currentMapID++;
    ...
    System.out.println(player.getName() + " awakes again on a " + currentField().getName() + ".\n" +
        ...
        +
        "revenge his death...\nBut " + player.getName()
        + " cannot show any fear! For he is the Dragon Slayer!\n" +
  ...
  private static void showManual() {
    ...
    System.out.println(
        "Try to defeat enemies and level up. Find new and better equipment to explore the region");
    System.out.println(
        "until you meet the almighty dragon boss. If you manage to win, you will advance to the");
    System.out.println(
        "next region. Your goal is defeating ten powerful dragons in ten progressively harder");
      ...
      switch (input) {
        case "1":
          ...
          if (playerAttack >= player.getWeapon().getAccuracy()) {   // Player hits enemy.
            ...
            System.out.println(
                player.getName() + " deals " + player.getWeapon().getAttack() + " damage " +
            ...
            if (currentField().getEnemy().getHealth() <= 0) {     // If we kill the enemy.
              ...
              if (!currentField()
                  .hasItem()) {  // Dead enemy drops it's weapon if the field is empty.
          ...
          if (enemyAttack >= currentField().getEnemy().getWeapon()
              .getAccuracy()) { // If the enemy hits.
            player
                .setHealth(player.getHealth() - currentField().getEnemy().getWeapon().getAttack());
        ...
        case "2":
          ...
          if (runChance >= threshold) {         // If you manage to get away.
            ...
            if (enemyAttack >= currentField().getEnemy().getWeapon()
                .getAccuracy()) { // If the enemy hits.
              player.setHealth(
                  player.getHealth() - currentField().getEnemy().getWeapon().getAttack());
              System.out.println(currentField().getEnemy().getName() + " dealt " +
                  currentField().getEnemy().getWeapon().getAttack() + " damage with it's " +
                  currentField().getEnemy().getWeapon().getName() + ".");
              if (player.getHealth() <= 0) {    // If the enemy kills us.
                ...
                System.out
                    .println(player.getName() + " has " + player.getHealth() + " health left.");
  ...
  private static void showStatus() {
    ...
    System.out
        .println("Health:\t\t\t\t" + player.getHealth() + " of " + player.getStartingHealth());
    ...
    System.out
        .println("Items in bag:\t\t\t" + player.getBagCounter() + " / " + player.getBag().length);
    System.out.println("Experience:\t\t\t" + player.getExperience());
    System.out.println("Level:\t\t\t\t" + player.getLevel());
    System.out.println("Enemies killed:\t\t\t" + player.getKillCount());
    System.out.println("Current mapID:\t\t\t" + (currentMapID - 1));
    System.out.println(
        "Current map size:\t\t" + map.getFields().length + ", " + map.getFields()[0].length);
    System.out
        .println("Current position:\t\t" + (player.getxPos() + 1) + ", " + (player.getyPos() + 1));
    System.out.println("");
  }

  /**
   * method to show a very rudimentary form of a map to get a faster understanding of the current
   * position.
   */
      ...
      for (int x = 0; x < width; x++) {
        ...
        if (y > 0 && y < height - 1) {         // y is inside bounds.
          ...
          if (x == 0) {                             // x is left side.
            ...
            if (((player.getxPos() * 2) + 1) == x
                && (player.getyPos() + 1) == y) {  // player's pos.
              System.out.print("X");
    ...
    switch (direction) {
      case "north":
        if (player.getyPos() > 0) {
          ...
          System.out.println(
              "You walk northwards and find yourself on a " + currentField().getName() + ".");
        } else {
      ...
      case "west":
        if (player.getxPos() > 0) {
          ...
          System.out.println(
              "You walk westwards and find yourself on a " + currentField().getName() + ".");
        } else {
      ...
      case "south":
        if (player.getyPos() < map.getFields()[0].length - 1) {
          player.setyPos(player.getyPos() + 1);
          System.out.println(
              "You walk southwards and find yourself on a " + currentField().getName() + ".");
        } else {
      ...
      case "east":
        if (player.getxPos() < map.getFields().length - 1) {
          player.setxPos(player.getxPos() + 1);
          System.out.println(
              "You walk eastwards and find yourself on a " + currentField().getName() + ".");
        } else {
      ...
      switch (singleInputs[0]) {
        ...
        case "delete":
          if (index < 0 || index > player.getBagCounter()) {
            ...
            System.out.println(
                player.getName() + " throws " + player.getBag()[index].getName() + " away.");
            if (!currentField()
                .hasItem()) {      // Item falls on the ground if no other item is there.
  ...
  private static void showHelp() {
    ...
    System.out
        .println(COMMANDS_WORLD[7] + " + index:\t\t\tdelete item at that index from your bag");
    ...
    System.out.println(COMMANDS_WORLD[11]
        + ":\t\t\t\topen an instruction manual with an explanation on how to play");
    System.out.println(
